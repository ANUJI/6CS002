...
private Domino exectutedFindGuessAndDomino(int x, int y, Domino d) {
  ...
  public void run() {
 //   IOSpecialist IOLibrary = new IOSpecialist();
...
//    System.out.println("Serial number " + Special.getStamp());
    ...
    while (_$_ != ZERO) {
      ...
      switch (_$_) {
      case 5:
        int index = (int) (Math.random() * (Fleetwood.chameleon.length / 3));
        String what = Fleetwood.chameleon[index * 3];
        String who = Fleetwood.chameleon[1 + index * 3];
      ...
      case 3: {
        ...
        JFrame f = new JFrame("Rules by Jamie Wood");
